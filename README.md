...
- **高負荷時の堅牢性（DNSリトライ＆起動遅延）**: 多数のクライアントをスケールアウト起動させた際のDNSサーバー過負荷や一時的なパケットロスによるエラーを防ぐため、DNS解決失敗時の「指数バックオフ＋ジッター付きリトライ（最大3回）」および仮想ユーザー起動時の「ランダム待機時間（0〜2000msのジッター）」を実装しています。
